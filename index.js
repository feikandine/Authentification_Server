...
//database credentials hidden in .env   ici  ce trouve les informations d'identification de la base de données
//  donc pour eviter d'ecrire directement les information exacte,on utilise "process.env.DB_HOST" ie process doit aller ds .env et metre  la valeur de  DB_HOST  "
...
const db = mysql.createPool({
  connectionLimit: 100,   // ici c'est pour limiter le nombre de requettes autoriser à un utilisateur
  host: DB_HOST,
  user: DB_USER,
...
db.getConnection((err, connection) => {
  ...
  console.log(
    ...
    attempt: ` + connection.threadId //  le + connection . threaId permet d'afficher le nobre de connection effectuer
  );
});
...
app.use(express.json()); // on demande à 
...
app.post("/createUser", async (req, res) => {
  ...
  //connection verfication db and inserting new user information ici on insert du sql directement pour que les informations ajoutés aille dans la base de donnée
...
// ici on essain de verifier, Authentifier la connexion de l'utilisateur
  ...
  db.getConnection(async (err, connection) => {
    ...
    //   si l'utilisateur n'existe pas dans la base de données
      ...
      if (result.length == 0) {
        ...
        //  obtenir le hashedPassword du résultat et le comparer à l'entrée de l'utilisateur 
